...
// Persists both preferences to localStorage so they survive page reloads.
// Applies the resolved theme as a class on <html> and writes accent CSS variables
// directly onto :root so every component picks them up via var(--primary).
...
// Writes all accent-related CSS variables onto :root so components update instantly.
...
// the existing <link> and append a fresh one to force an immediate re-fetch.
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // accent is in deps so the handler closure always has the latest accent.
  useEffect(() => {
