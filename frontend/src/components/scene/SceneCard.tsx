...
// switches to an animated webp or mp4 preview on hover/touch to mimic the
// behaviour of modern video-sharing platforms.
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ aspectRatio: "16/9", backgroundColor: "var(--bg-secondary)" }}
      >
        ...
        {showPreview &&
          (hoverVideo ? (
            ...
            <img src={hoverWebp!} alt={title} className="absolute inset-0 w-full h-full object-cover" />
          ))}
        ...
        {/* Watched badge - shown when play_count is non-zero */}
        ...
        <div className="flex flex-col gap-0.5">
          ...
          {scene.studio && (
            <span
              role="link"
              tabIndex={0}
              onClick={(e) => { e.preventDefault(); e.stopPropagation(); navigate(`/studios/${scene.studio!.id}`); }}
              onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); e.stopPropagation(); navigate(`/studios/${scene.studio!.id}`); } }}
          ...
          {/* Performers - each name individually clickable to avoid nested <a> */}
              ...
              {scene.performers.map((p, i) => (
                <span key={p.id}>
                  <span
                    role="link"
                    tabIndex={0}
                    onClick={(e) => { e.preventDefault(); e.stopPropagation(); navigate(`/performers/${p.id}`); }}
                    onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); e.stopPropagation(); navigate(`/performers/${p.id}`); } }}
          ...
          <div className="flex items-center gap-2 text-xs" style={{ color: "var(--text-muted)" }}>
            {scene.play_count != null && <span>{formatViews(scene.play_count)}</span>}
