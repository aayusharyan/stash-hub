// Performer detail page. Shows a hero banner with the performer's photo, bio,
// physical details, social links, and a paginated grid of their scenes.
...
import { usePerformer, useScenes } from "@/lib/queries";
...
// Stat-level chip for country/nationality. Shows one clickable SVG flag per
// nationality, each linking to a filtered scene search.
    ...
    <div className="flex flex-col items-center justify-between px-4 py-2 rounded gap-1" style={{ backgroundColor: "var(--bg-secondary)" }}>
      <div className="flex items-center gap-2">
        {flags.map(({ iso, label }) => (
          <Link
            key={iso}
            to={`/search?type=videos&performer_country=${encodeURIComponent(label)}`}
            title={label}
            className="transition-opacity hover:opacity-70"
            style={{ lineHeight: 0 }}
          >
            <ReactCountryFlag countryCode={iso} svg style={{ width: "1.6em", height: "1.2em" }} />
          </Link>
        ))}
      </div>
...
export default function PerformerPage() {
  ...
  const { data: performer, isLoading: perfLoading } = usePerformer(id);
  ...
  const total = scenesData?.count ?? 0;
        ...
        <div className="mx-auto w-full max-w-screen-2xl px-3 md:px-6">
          <div className="py-8 flex flex-col sm:flex-row items-start gap-6">
            ...
            <div className="flex-1 min-w-0">
              ...
              {performer.alias_list && performer.alias_list.length > 0 && (
